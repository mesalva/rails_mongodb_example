...
EXPOSE 3000
CMD /bin/bash -c -l 'source /etc/profile.d/rvm.sh && git pull origin develop && HOME=/ranking_points bundle update && HOME=/ranking_points bundle install --path vendor/cache && HOME=/ranking_points RAILS_ENV=production rails s'