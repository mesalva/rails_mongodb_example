...
EXPOSE 3000
CMD /bin/bash -c -l 'source /etc/profile.d/rvm.sh && git pull origin master && HOME=/ranking_points bundle update && HOME=/ranking_points bundle install --path vendor/cache && RAILS_ENV=production bundle exec unicorn -p 3000'